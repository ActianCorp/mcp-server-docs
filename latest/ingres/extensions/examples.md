---
title: Examples
description: Runnable example extensions for the Actian MCP Server, with configuration and schema files for Ingres.
---
...
Ingres has a configuration template and a schema file.
...
The schema file creates seven tables and seed data, and enforces the rule that stock cannot go negative with a `CHECK` constraint. The schema file's header lists the grant statements to run afterwards for the account the server connects as.
