---
title: Examples
description: Runnable example extensions for the Actian MCP Server, with the configuration and schema files for HCL Informix.
---
...
HCL Informix has a configuration template and a schema file.
...
The schema file creates seven tables and seed data. The rule that stock cannot go negative is enforced with a `CHECK` constraint. The schema file's header lists the grant statements to run afterwards for the account the server connects as.
