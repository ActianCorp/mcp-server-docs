...
Extensions are supported on Actian Ingres.
...
Statements run as the end user. On Ingres, your statements run as the authenticated user, not as the server's service account.
...
- :material-file-code: **[Examples](examples.md)**  
  Five runnable extensions, with a configuration template and a schema file.
